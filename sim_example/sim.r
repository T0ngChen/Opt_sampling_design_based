...
one.sim = function(beta, N = 4000, n = 600, r = 0.9, err = 1.5){
  ## generate data
  ...
  fit = lm(infl~df$X_tilde)
  infl2 = residuals(fit)
  infl3 = infl2
  infl3[strata[[2]]]  = infl3[strata[[2]]] * err
  
  infl4 = infl - infl3
  infl5 = residuals(lm(formula = infl~infl4))
  df$infl4 = infl4
  ...
  for(kk in 1:length(strata)){
    sd.stra2 = c(sd.stra2, sd(infl5[strata[[kk]]]))
  }
  ...
  list(coef.ipw = coef.ipw, coef.rak = coef.rak,
       ...
       sample.size = sample.size, corr = cor(infl, infl4))
}



out = replicate(2000, one.sim(beta = rep(1,3), err = 3, r = 0.9553))
