...
one.sim<-function(beta, N = 4000, n = 600, err){
  ## generate data
  df <- data.frame(Z1 = rbinom(N, 1, .5))
  df$Z2 <- runif(N)
  df$X <- rnorm(N, 0, 1)
  epsilon <- rnorm(N, 0, 1)
  df$Y <- beta[1] + beta[2]*df$X + beta[3]*df$Z1 + beta[4]*df$Z2 + epsilon
  U = rnorm(N, 0, err)
  df$X_tilde <- df$X + U
  df$id<-1:nrow(df)
  ...
  df$zstrat<-cut(df$X_tilde, c(-Inf, quantile(df$X_tilde, probs = c(0.20, 0.80)), Inf))
  df$stra = as.numeric(df$zstrat)
  n1 = xtabs(~stra, df)
  strata = list()
  ...
  ##                       ##
  ###########################
  infl = dfbeta(lm(Y~X+Z1+Z2,data=df))[, 2]
  sd.stra1 = numeric()
  ...
  ini$predictorMatrix["X", c(1:2,4:5)] = 1
  meth = ini$method
  meth[meth!=""] = ""
  meth["X"] = "norm"
