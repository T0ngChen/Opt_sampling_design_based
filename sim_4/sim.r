...
one.sim<-function(beta, N = 10000, se, sp){
  ## generate data
  ...
  df$X_tilde = ifelse(df$X == 1, rbinom(N,1,se), rbinom(N,1,sp))
  ...
  df3$insample = (1:nrow(df3)) %in% unlist(s.scc)
  SCC = impute.raking(df3)
  ipw.SCC = SCC[[1]]
  rak.SCC = SCC[[2]]
  ...
  imp.full <- glm(X~Y+Z+X_tilde,family=binomial, data=df1)
  inffun_mi<-matrix(0,nrow=nrow(df1),ncol=3)
  M = 50
  for(i in 1:M){
    df1$impx <- rbinom(nrow(df1), 1, as.vector(predict(imp.full, newdata=df1, type="response", se.fit=FALSE)))
    phase1model.imp <- glm(Y~impx+Z, family = binomial, data=df1)
  ...
  #}
  #  
  #df3$insample = (1:nrow(df5)) %in% unlist(s.bss)
  #BSS = impute.raking(df5)
  #ipw.BSS = BSS[[1]]
  #rak.BSS = BSS[[2]]
  ...
  coef.ipw = c((coef(ipw.SRS) - beta)[2], (coef(ipw.SCC) - beta)[2], 
               (coef(ipw.PSS) - beta)[2], (coef(ipw.IFIPW) - beta)[2],
               (coef(ipw.IFGR) - beta)[2], (coef(ph.two.srs) - beta)[2])
  
  coef.rak = c((coef(rak.SRS) - beta)[2], (coef(rak.SCC) - beta)[2], 
  ...
  sample.size = cbind(ney, optimal.rak, scc.num)
...
out = replicate(2000, one.sim(beta = beta, se = se, sp = sp))
